import time
from stm_alert.display.drivers.raspberry_pi import RaspberryPi
from stm_alert.display.constants import LEFT_SCREEN, RIGHT_SCREEN
import numpy as np


class AdjacentLCD(RaspberryPi):
    ...
    def __init__(self, which_screen: str):
        match which_screen:
            case "left":
                self.screen_config = LEFT_SCREEN
            case "right":
                self.screen_config = RIGHT_SCREEN
            case _:
                raise ValueError(f"Invalid screen. Please specify 'left' or 'right'.")

        super().__init__(**self.screen_config)
