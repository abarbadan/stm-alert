...
class Trainer:
    ...
    def gaussian_nll_loss(
        self, pred_mean: torch.Tensor, pred_sigma: torch.Tensor, target: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        nll = 0.5 * torch.log(2 * np.pi * pred_sigma**2) + 0.5 * (
            (target - pred_mean) ** 2
        ) / (pred_sigma**2)
    ...
    def reconstruction_loss(
        self, pred_reconstruction: torch.Tensor, inputs: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def train_epoch(self, train_loader: DataLoader) -> Tuple[float, float, float]:
        ...
        return (np.mean(total_losses), np.mean(nll_losses), np.mean(recon_losses))
